...
def trackerIterator(roi, img, similarityThresh, maxIterations, iterations, dissimilarityThresh, similarityBreakThresh, convergeThresh):
	...
	#feature: img2(x,y,width,length,offset,inverted Hessian)
...
while(True):
	...
	for feature in roi.features:
		...
		cv2.rectangle(showcaseImg,(int(rectangle[0]),int(rectangle[1])),(int(rectangle[2]),int(rectangle[3])),(255,255,255),1)
